...
def process_experiments(exps):
    ...
    for experiment_no in exps:
        ...
        for index,file in tqdm.tqdm(enumerate(h5_files)):
            try:
                ...
                for channel in range(h5['ai_channels'].shape[-2]):
                    if len(glob.glob(file_path+f"nidaq/acquisition_data_{index}_{channel}.wav")) == 0:
        ...
        threshold_ma_stop = 0.7 # change to 1.6 if it's too sensitive a threshold
        ...
        threshold_ma_start = -1 # This threshold is based on difference in moving average from one index to the next
        threshold_ma_stop = 0.7 # This threshold is based on difference in moving average from one index to the next


        # Getting the indices of the start and stop record
        print("Getting the index for start record")
        indices_start = []
        idx3 = indices_r[start_record_file_no][3:] # indices of the rising edges from the 3rd index onwards
        val = None
        for counter,(idx1,idx2) in tqdm.tqdm(enumerate(zip(indices_r[start_record_file_no][:-1],indices_r[start_record_file_no][1:]))):
            diff = MA[0][idx2]-MA[0][idx1]
            if diff < threshold_ma_start:
                print("Difference: ",diff)
                try:
                    val = MA[0][idx3[counter]]
                    print("Value: ", val)
                except: # if the edge is towards the end of the file
                    pass
                if val:
                    if val < -0.5:
                        print("Set value of indices start")
        ...
        with open(path_length_log, "w") as f:
            ...
            message = f"The camera names are:\n{camera_names}\n"
            f.write(message)
            ...
            for item in tqdm.tqdm(video_by_camera.items()):
                ...
                message = f"Processing Camera: {item[0]}\n"
                f.write(message)               
                ...
                for idx,vid in enumerate(item[1]):
                    ...
                    if idx == len(item[1])-1:   # we overrite the length with the imutils length for the last video because it is more accurate
                        message = f"The total number of frames are: {tot_length+length}\n"
                        f.write(message)
            ...
            f.write("The camera lengths are as follows\n")
            f.write(f"{vid_lengths}\n")
            ...
            if len_nidaq < len_video:
                diff = len_video - len_nidaq
                message = f"Video data is longer by {diff} frames\n"
                print(message.strip())
                f.write(message)
                video_data = video_data.iloc[:len_nidaq, :]

            elif len_video < len_nidaq:
                diff = len_nidaq - len_video
                message = f"NIDAQ data is longer by {diff} samples\n"
                print(message.strip())
                f.write(message)
                nidaq_data = nidaq_data.iloc[:len_video, :]

            else:
                message = "Both datasets are equal in length\n"
                print(message.strip())
                f.write(message)
            ...
            f.write(f"Removed the last {crop_len} seconds\n")
...
# sample run script: python timestamp_generation_gily_automated_flatiron.py 493 494 495
