...
mic_deque_1 = deque(maxlen=spec_buffer_len) # 3 seconds worth
mic_deque_2 = deque(maxlen=spec_buffer_len) # 3 seconds worth
mic_deque_3 = deque(maxlen=spec_buffer_len) # 3 seconds worth
mic_deque_4 = deque(maxlen=spec_buffer_len) # 3 seconds worth
# mic_deque_5 = deque(maxlen=spec_buffer_len) # 3 seconds worth
...
def calc_spec_frame_segment_mono(all_audio):
    ...
    # spec_5 = 20*np.log10(spec_5+1e-12)
    # print("Before cutoff")
    ...
    # spec_5 = np.clip(spec_5, minavg, maxavg)
    # spec_5 = (spec_5 - minavg) * 255 / (maxavg - minavg)
...
def spec_plot(read_buffer,read_idx,flag_end):
    ...
    while flag_end.value == 0:
        ...
        #-----------------------------------------------------------------pre run check (spectrogram plotting) -------------------------------------------------------------------------
                    color_frame_1,color_frame_2,color_frame_3,color_frame_4 = calc_spec_frame_segment_mono(final_arr[:-1,(read_idx.value-1)*chunk_size:read_idx.value*chunk_size])
...
def digital_in_slot_2(read_buffer,flag_end,read_idx,flag_reset):
    ...
    with nidaqmx.Task() as tsk:
        ...
        if len(channels_di_slot_2) > 0:
            ...
            # tsk.timing.cfg_samp_clk_timing(sampling_rate, sample_mode=nidaqmx.constants.AcquisitionType.CONTINUOUS)
