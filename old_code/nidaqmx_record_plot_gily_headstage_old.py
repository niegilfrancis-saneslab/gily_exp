...
channels = ["PXI1Slot2/ai0", # nest - mic 4
            ...
            "PXI1Slot4/ai3", # headmounted Mic 1 TTL Sanes Lab Transceiver A
            "PXI1Slot4/ai4", # headmounted Mic 2 TTL Schneider Lab Transceiver 
...
spec_buffer_len = 600 # number of chunks - 576 is 3 seconds worth
...
def spec_plot(read_buffer,read_idx,flag_end):
    ...
    while flag_end.value == 0:
        ...
        #-----------------------------------------------------------------pre run check (spectrogram plotting) -------------------------------------------------------------------------
                    color_frame  = calc_spec_frame_segment_mono(final_arr[:-(1+len(channels_di_slot_2)),(read_idx.value-1)*chunk_size:read_idx.value*chunk_size])
    ...
    with nidaqmx.Task() as task:
        ...
        try:
            while flag_end.value == 0: 
                ...
                final_arr[:len(channels),read_idx.value*chunk_size:(read_idx.value+1)*chunk_size] = chunks
